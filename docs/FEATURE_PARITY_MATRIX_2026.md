...
| Legacy routes | `PRESENT` - `/channel/:uuid`, `/channel/:uuid/subs`, `/channel/:uuid/followers`, `/channel/:uuid/queue`. | `PRESENT` - all legacy routes preserved. | `MISSING` - new app uses new widget routes, not `/channel/:uuid`. | Keep in bridge only; do not force into new app unless migration needs redirects. | P0 bridge | OBS compatibility depends on these routes. |
