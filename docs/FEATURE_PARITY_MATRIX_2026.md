...
| Legacy `/channel/*` routes | `PRESENT` - `/channel/:uuid`, `/channel/:uuid/subs`, `/channel/:uuid/followers`, `/channel/:uuid/queue`. | `REMOVED` - current `overlays-legacy` rejects `/channel/*` and `/test/channel/*`. | `MISSING` - new app uses explicit widget routes, not `/channel/:uuid`. | Historical only. Do not document as active runtime behavior. | P0 docs | OBS sources must be migrated to explicit routes. |
| Explicit overlay routes | `MISSING` in original build. | `PRESENT` - `/ALERTS/:uuid`, `/TIP_ALERT/:uuid`, `/REWARD_ALERT/:uuid`, `/SUB_GOAL/:uuid`, `/FOLLOW_GOAL/:uuid`, `/QUEUE/:uuid`; `/` renders the link generator. | `PRESENT / DIFFERENT` - new app supports typed/widget routes with a different auth model. | Use explicit routes as the only active `overlays-legacy` route contract. | P1 | Routes still use legacy `?account=:uuid`; `?test=true` enables runtime test mode. |
