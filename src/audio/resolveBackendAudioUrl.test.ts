...
describe('resolveBackendAudioUrl', () => {
  it('keeps absolute https URLs unchanged', () => {
    expect(resolveBackendAudioUrl('https://cdn.example.com/file.mp3')).toBe('https://cdn.example.com/file.mp3');
  });

  it('keeps absolute http URLs unchanged', () => {
    expect(resolveBackendAudioUrl('http://cdn.example.com/file.mp3')).toBe('http://cdn.example.com/file.mp3');
  });

  it('normalizes protocol-relative URLs to https', () => {
    expect(resolveBackendAudioUrl('//cdn.example.com/file.mp3')).toBe('https://cdn.example.com/file.mp3');
  });
