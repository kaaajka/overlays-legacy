...
export default class RouletteEvent extends React.Component<IRouletteEventProps, {}> {
    ...
    render() {
        ...
        // @ts-ignore
        const winningItem = event.winner && this.finished ? event.items.find( item => item.start <= event.winner && item.end >= event.winner) : null;
    ...
    private startRoll(winningTicket: number) {
        ...
        const elements = Array.from(this.rollerRef.current.querySelectorAll(".o[data-start][data-end]").values()).filter( element => {
        ...
        const index = Array.prototype.slice.call(this.rollerRef.current.children).indexOf(randomElement);
