...
describe("main overlay mode filtering", () => {
  it("accepts only normal main event names in normal mode", () => {
    for (const event of [
      ...
      expect(shouldHandleMainOverlayEventName(event, false)).toBe(true);
    }
    ...
    for (const event of [
      ...
      expect(shouldHandleMainOverlayEventName(event, false)).toBe(false);
    }
  });

  it("accepts only test main event names in test mode", () => {
    for (const event of [
      "test",
      "t_prepare",
      "t_started",
      ...
      expect(shouldHandleMainOverlayEventName(event, true)).toBe(true);
    }
    ...
    for (const event of [
      ...
      expect(shouldHandleMainOverlayEventName(event, true)).toBe(false);
    }
  });
  ...
  it("keeps all mode independent from optional origin metadata", () => {
    ...
    expect(shouldHandleMainOverlayEvent("all", "unknown-key", "reward")).toBe(
      true,
    );
  });
  ...
  it("keeps tip mode key-based even when optional origin metadata is present", () => {
    ...
    expect(shouldHandleMainOverlayEvent("tip", "roulette", "reward")).toBe(
      false,
    );
  });
  ...
  it("prefers reward origin in reward mode when origin metadata is present", () => {
    expect(shouldHandleMainOverlayEvent("reward", "donate", "reward")).toBe(
      true,
    );
    expect(
      shouldHandleMainOverlayEvent("reward", "unknown-key", "reward"),
    ).toBe(true);
    expect(shouldHandleMainOverlayEvent("reward", "roulette", "reward")).toBe(
      true,
    );
  });

  it("rejects manual origin in reward mode when origin metadata is present", () => {
    expect(shouldHandleMainOverlayEvent("reward", "donate", "manual")).toBe(
      false,
    );
    expect(shouldHandleMainOverlayEvent("reward", "roulette", "manual")).toBe(
      false,
    );
    expect(
      shouldHandleMainOverlayEvent("reward", "unknown-key", "manual"),
    ).toBe(false);
  });
});
